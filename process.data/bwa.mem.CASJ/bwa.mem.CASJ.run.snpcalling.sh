...
#$ -pe shared 15             #specify number of CPUs requested
...
#used the following code 'wget https://ftp.ncbi.nlm.nih.gov/genomes/all/GCA/028/536/675/GCA_028536675.1_bAphCal1.0.hap1/GCA_028536675.1_bAphCal1.0.hap1_genomic.fna.gz'
#to download this assembly: https://www.ncbi.nlm.nih.gov/datasets/genome/GCA_028536675.1/ of a California Scrub-jay
#then renamed it to 'CASJ.fna.gz' and ran: 'gunzip CASJ.fna.gz'
#index for bwa
#/u/project/aguillon/shared_bin/bwa-mem2-2.2.1_x64-linux/bwa-mem2 index CASJ.fna
#index for picard (v3.3.0)
#java -jar /u/project/aguillon/shared_bin/picard/build/libs/picard.jar CreateSequenceDictionary R=CASJ.fna O=CASJ.fna.dict
#index for samtools
#samtools faidx CASJ.fna

# define the reference genome
refgenome=/u/home/d/dderaad/project-aguillon/eagle.mountain.scrubjays/CASJ.fna
...
#        --length_required 75 \ #remove trimmed reads below 75 base-pairs
...
# run bwa mem (v2.2.1) on the reads cleaned with fastp and pipe output to samtools to construct sorted bam file
/u/project/aguillon/shared_bin/bwa-mem2-2.2.1_x64-linux/bwa-mem2 mem -t ${threads} ${refgenome} \
/u/home/d/dderaad/project-aguillon/eagle.mountain.scrubjays/cleaned_reads/${sample}-READ1.fastq.gz \
/u/home/d/dderaad/project-aguillon/eagle.mountain.scrubjays/cleaned_reads/${sample}-READ2.fastq.gz | samtools sort -o bam_files/${sample}.bam
...
#filter to remove reads mapped with an insert size less than the length of a single read (150 bp) which are likely spurious
