...
### First run: test 3 CASJ individuals as hybrids (they will be run with 99% estimated CASJ ancestry)
...
Aph_californica_65020	2
Aph_californica_333971	2
Aph_californica_334041	2
EOF
...
conda activate ancestryHMM
#run the program
ancestry_hmm -i diagnostic.HMM.input.txt -s sample_list.txt -a 2 0.999 0.001 -p 0 100000 0.999 -p 1 -100 0.001 -o test.cali
...
### Run #2: repeat this time with woodhouse's individuals as the hybrids
...
#define hybrid samples with ploidy for ancestryHMM input
cat > sample_list.txt << 'EOF'
Aph_woodhouseii_334081	2
Aph_woodhouseii_334080	2
Aph_woodhouseii_27904	2
EOF
...
### Step 3: run ancestryHMM
#run the program
ancestry_hmm -i diagnostic.HMM.input.txt -s sample_list.txt -a 2 0.001 0.999 -p 1 100000 0.999 -p 0 -100 0.001 -o test.wood
...
###Run #3: create a synthetic F1 hybrid by combining the reads from two parental samples that were sequenced at equal depth: Aph_woodhouseii_334080 + Aph_californica_195411
...
#now we will combine the reads from both samples into one artificial F1
#replace column 8 (ref alleles from the first individual) with the sum of columns 8 and 10 (ref alleles from both samples)
awk 'BEGIN{FS=OFS="\t"} {$8 = $8 + $10; print}' diagnostic.HMM.input.txt > tmp && mv tmp diagnostic.HMM.input.txt
#replace column 9 (alt alleles from the first individual) with the sum of columns 9 and 11 (alt alleles from both samples)
awk 'BEGIN{FS=OFS="\t"} {$9 = $9 + $11; print}' diagnostic.HMM.input.txt > tmp && mv tmp diagnostic.HMM.input.txt
#remove columns 10 and 11, leaving just the single artificial hybrid sample
cut --complement -f10,11 diagnostic.HMM.input.txt > tmp && mv tmp diagnostic.HMM.input.txt

### Step 3: run ancestryHMM
#run the program
ancestry_hmm -i diagnostic.HMM.input.txt -s sample_list.txt -a 2 0.5 0.5 -p 1 100000 0.5 -p 0 -1 0.5 -o test.F1
